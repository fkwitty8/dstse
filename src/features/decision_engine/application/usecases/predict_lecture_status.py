...
from typing import Dict
from features.decision_engine.domain.entities import Lecture, WeatherPrediction, TrafficPrediction, LectureDecision
from features.decision_engine.domain.repositories import BayesianNetworkRepository, SVMPolicyRepository
from features.decision_engine.infrastructure.clients.weather_client import WeatherAPIClient
...
class PredictLectureStatusUseCase:
    ...
    def execute(self, lecture: Lecture) -> LectureDecision:
        ...
        # Compute delay probability (e.g., sum of disruptive states)
        delay_prob = sum(decision_probs.get(state, 0) for state in ['Online', 'Rescheduled', 'Cancelled','Scheduled'])
